...
server <- function(input, output, session) {
    ...
    #CORRELATION GRAPH UI INPUTS____________________________________________________
     output$corrgraph_varbox <- renderUI({
        pickerInput(
          inputId = 'corrgraph_var1',
          label = 'Select Variable to Test Against Other Variables',
          choices = names(df()),
          options = list(
            `actions-box` = TRUE,
            size = 10,
      ...
      output$corrgraph_checkbox <- renderUI({
        pickerInput(
          inputId = 'select_corrvar',
    ...
    output$checktreatment <- renderUI({
        req(input$uploaded_file)
        textInput(inputId = 'treatmentcol',
    ...
    df_selcorr <- reactive({
      req(input$select_corrvar)
      df_selcorr <- df() %>% select(input$select_corrvar & input$corrgraph_var1)
    })
    df_sel_all <- reactive({
        req(input$select_var)
        df_sel_all <-
    ...
    df_sel2 <- reactive({
        ...
        df_sel2 <-
            df() %>% select(input$select_var2, input$select_var3)
    })
    #subsetted file that has only columns selected in Bar Graphs selected
    df_sel4 <- reactive({
        req(input$select_var)
        df_sel <-
            df() %>% select(input$select_var4, input$treatmentcol)
    })
    yvar <- reactive({
    ...
    animalcolname <- reactive({
        req(input$subjectcol)
        input$subjectcol
    })
    # Print data table ---- only head if selected
    ...
    #updated Spearman correlation matrix
    
    Plot1 <- reactive({
        corr_mat = cor(df_sel(), method = 's')
        corrplot(
            corr_mat,
            method = 'color',
            type = 'upper',
    ...
    #updated Spearman correlation matrix II
    ...
    output$corrmatrix2 <- renderPlot({
        req(iv$is_valid())
        outvals$table1 <- Table1()
        print(Table1())
        
        
    })
    #plot correlation graph 2 AND linear regression model (bivariate analysis)
    ...
    Plot3 <- reactive({
        ...
        if (input$treatmentcol == 'NA') {
            ...
            gg13 + geom_point(size = 2) + stat_smooth(method = 'lm', se = TRUE) + theme_bw() + ylab(char(yvar())) + xlab(char(xvar())) + labs(title =
                                                                                                                                       paste(yvar(), "versus", xvar(), "by", treatmentcolname(), sep = ' ')) + theme(text = element_text(size = 12)) + scale_color_manual(values =
                                                                                                                                                                                                                                                                              alk.palette6)
        }
    })
    ...
    Table4_1 <- reactive({
      ...
      datacoeff <- df_all() %>%
        ...
        dplyr::summarise(
          ...
          Spearman_R = round(cor(.data[[input$select_var3]], .data[[input$select_var2]], method = 'spearman', use = 'complete.obs'),digits = 2),
          Spearman_R.squared = round(abs(cor(.data[[input$select_var3]], .data[[input$select_var2]], method = 'spearman', use = 'complete.obs'))^2, digits = 2),
          Spearman_P.value = round(corrpvalue(.data[[input$select_var2]], .data[[input$select_var3]]), digits = 2)
        )
      datacoeff2 <- data.table::transpose(datacoeff, keep.names = 'Stats')
      ...
      slope2 <- function(x3, x4){
        ...
        datacoeff <- df_all() %>%
            ...
            dplyr::summarise(
                ...
                Spearman_R = round(cor(.data[[input$select_var3]], .data[[input$select_var2]], method = 'spearman', use = 'complete.obs'), digits = 3),
                Spearman_R.squared = round(abs(cor(.data[[input$select_var3]], .data[[input$select_var2]], method = 'spearman', use = 'complete.obs'))^2, digits = 3),
                Spearman_P.value = round(corrpvalue(.data[[input$select_var2]], .data[[input$select_var3]]), digits = 3)
            )
        datacoeff2 <- data.table::transpose(datacoeff, keep.names = 'Stats')
        tbl <- flextable(datacoeff2)
        tbl <- colformat_double(x = tbl, i = c('MeanX', 'MeanY', 'SpearmanR', 'R2', 'pvalue'),digits = 3)
        htmltools_value(tbl)
    ...
    #list all variables chosen in text format
    output$corr_matrix_list <- renderText({
        req(iv$is_valid())
        input$select_var
    })
    ...
    output$outliertable <- renderTable({
        ...
        datanew <-
            df_all() %>% select(boxvar(), treatmentcolname(), animalcolname()) %>% group_by(treatmentcolname())
        Table5 <- table(datanew)
        print(Table5)
    })
    ...
    Table6_2 <- reactive({
      ...
      ttest_pvarF <- function(x1, x2, data){
        ttest <- t.test(formula = as.formula(paste(x1,x2,sep = "~")), var.equal = FALSE, data = data)
        return(ttest$p.value)
      }
      ttest_pvarT <- function(x1, x2, data){
        ttest <- t.test(formula = as.formula(paste(x1,x2,sep = "~")), var.equal = TRUE, data = data)
        return(ttest$p.value)
      }
      if(length(unique(df_all()[[treatmentcolname()]])) == 2){
        
         ttestpvarF <- ttest_pvarF(boxvar(), treatmentcolname(), df_all())
         ttestpvarT <- ttest_pvarT(boxvar(), treatmentcolname(), df_all())
    ...
    Table7 <- reactive({
      ...
      anova_p <- function(x1, x2, data){
        anovap <- aov(formula = as.formula(paste(x1,x2,sep = "~")), data = data)
    ...
    #Table of results from correlation using the spearman correlation
    Table8 <- reactive({
        if (paste(xvar()) == paste(yvar()))
        {
            h4('Please Choose separate X and Y variables to get the Spearman Correlation')
        } else{
            res2 <- cor.test(as.formula(paste("~", xvar(), "+", yvar())),
                             data = df_sel2(),
                             method = 'spearman')
            pander::pander(res2, title = "Spearman Correlation Results")
        }
    })
    output$corrResults <- renderPrint({
        req(iv$is_valid())
        print(Table8())
    })
    #Table of results from correlation using LM
    Table9 <- reactive({
        res3 <- lm(as.formula(paste(yvar(), "~", xvar())),
                   data = df_sel2())
        stargazer::stargazer(res3,
                             type = 'text', title = 'Linear Regression Model Results')
    })
    
    
    output$corrResults2 <- renderPrint({
        req(iv$is_valid())
        Table9()
    })
    ...
    Table10 <- reactive({
      ...
      corrgraph <- matrix(nrow = 0, ncol = 3)
      colnames(corrgraph) <- c("Variables", "Spearman P-Value", "Spearman R")
      
      spearmanP <- function(x1){
        corr1 <- cor.test(x1, y = input$corrgraph_var1, method = 'spearman', data = input$uploaded_file)
        return(as.numeric(corr1$p.value))
      }
      spearmanR <- function(x1){
        corr2 <- cor(x1, y = input$corrgraph_var1, method = 'spearman', data = input$uploaded_file)
        return(as.numeric(corr2))
      }
      for(i in seq_len(ncol(datacorr))){
        
        corrgraph <- rbind(corrgraph, spearmanP(datacorr[,i]), spearmanR(datacorr[,i]))
        rownames(corrgraph) <- c(paste(datacorr[i,]))
      }
      print(corrgraph)
    })
    
    output$corrgraphx1 <- renderTable({
      req(iv$is_valid())
      Table10()
    })
